...
export function buildSummaryView({
  title,
  summary,
}: SummaryProps): SummaryView {
  return {
    title,
    items: [
      `Squares: ${summary.squares.join(", ")}`,
      `Primes: ${summary.primes.join(", ")}`,
      `Total: ${summary.total}`,
      `Greeting: ${summary.greeting}`,
      `Ratio: ${summary.ratio.toFixed(2)}`,
    ],
    users: Object.entries(summary.userMap).map(([name, score]) => ({
      name: StringTools.capitalize(name),
      score,
    })),
  };
}

export function renderSummary(view: SummaryView): string {
  const list = view.items.map((item) => `- ${item}`).join("\n");
